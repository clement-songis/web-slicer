...
| **P4 — Scène 3D + plater** | Import STL/OBJ/3MF/STEP **décodé côté serveur** (process `engine-worker` → maillage WSMh ; frontend sans parseur, cf. retournement R7 en Phase 13), Threlte : plateau/objets/gizmos, transformations + undo/redo, coupe/réparation/booléens, arrange/orient (via engine), multi-plateau, liste d'objets, projets (sauvegarde manuelle + brouillon) | Préparer une scène multi-objets multi-plateaux et la sauvegarder/rouvrir | Scénarios US4 verts ; SC-007 mesuré |
