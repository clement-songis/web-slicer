...
- [~] T028 [US6] Auth `backend/src/auth/` : **service fait** — argon2id (`password.rs`), `register` selon `registration_policy` (open/closed/invite + invitations usage unique), bootstrap premier compte = admin, `authenticate` (`service.rs`) ; tests verts (`auth_service.rs` + unités) : hash jamais en clair, politiques, identifiants. **Reste** : couche tower-sessions (store sqlx) + endpoints login/logout/me (avec l'assemblage du routeur/AppState, T029/T030)
